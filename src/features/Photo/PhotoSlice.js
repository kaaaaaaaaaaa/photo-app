...
const photoReducer = createSlice({
  ...
  reducers: {
    ...
    removePhoto: (state, action) => {
      ...
      //filter lấý lại cacc gt có id != removePhotoId
      return (state = state.filter((photo) => photo.id !== removePhotoId));
    },
    updatePhoto: (state, action) => {
      const newPhoto = action.payload;
      //kt index của item can update
      const photoIndex = state.findIndex((photo) => photo.id === newPhoto.id); //k tim dc =>-1
      if (photoIndex >= 0) {
...
const { reducer, actions } = photoReducer;
